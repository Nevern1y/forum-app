-- ============================================================================
-- АЛЬТЕРНАТИВНОЕ РЕШЕНИЕ: Функция БЕЗ LATERAL JOIN
-- ============================================================================
-- Это решение использует подзапрос вместо LATERAL JOIN
...
BEGIN
  ...
  SELECT
    ...
    -- ИСПРАВЛЕНИЕ: Используем подзапрос вместо LATERAL JOIN
    (
      SELECT COALESCE(json_agg(json_build_object('name', t.name)), '[]'::json)
      FROM post_tags pt
...
-- ============================================================================
-- ВАЖНО: Если эта версия тоже выдаёт ошибку, значит проблема в таблице tags
-- Выполните CHECK_TAGS_STRUCTURE.sql чтобы проверить структуру таблиц
-- ============================================================================
